...
SCP 是一个构建在 UDP 或任何不可靠传输之上的轻量级可靠流协议，专为需要明确行为、可控传输和可调拥塞控制的场景而设计。它不是对 TCP 的简化，而是在许多实际环境中能够提供更稳定、更可预测传输性能的独立协议。

相比完整的 TCP，SCP 的实现更轻量、更容易定制，特别适合嵌入式设备、用户态网络栈以及需要实验新控制算法的系统。其核心特点是 **非线性类 AIMD 拥塞控制算法**：通过丢包偏差、RTT 偏差及其交互项构造拥塞信号，经概率映射后驱动窗口的加性增大与概率化减小，使协议在高丢包、无线抖动等复杂环境中保持稳定与可控的传输性能。

SCP 实现了序号、ACK、SACK、重传、窗口、握手与关闭等必要机制，同时保持极小的代码规模和清晰的状态机结构，便于阅读、移植和扩展，适合在用户态、嵌入式或内网环境中直接使用。
...
- 采用非线性类 AIMD 拥塞控制算法，结合丢包偏差与 RTT 偏差生成平滑的拥塞信号，适用于高丢包与无线场景
